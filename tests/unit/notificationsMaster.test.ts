...
jest.mock('../../lib/notifications/weeklyReview', () => ({
  reconcileWeeklyReview: jest.fn().mockResolvedValue(undefined),
}));

describe('applyNotificationsMaster', () => {
  beforeEach(() => jest.clearAllMocks());

  it('persists the pref and reconciles all five categories', async () => {
    ...
    // WR-4: the empty query cache reads as zero active goals here.
    expect(reconcileWeeklyReview).toHaveBeenCalledWith(false);
  });
});
